...
test("scheduler reuses same-root jobs and enforces global concurrency", async () => {
  ...
  const first = scheduler.submit({ canonicalRoot: "/repo-a", reason: "manual", run });
  const duplicate = scheduler.submit({ canonicalRoot: "/repo-a", reason: "manual", run });
  const second = scheduler.submit({ canonicalRoot: "/repo-b", reason: "manual", run });
...
test("scheduler keeps one follow-up for changes submitted while a root is running", async () => {
  ...
  const reused = scheduler.submit({
    canonicalRoot: "/repo-a",
    reason: "watch",
    run: async () => { runs.push("follow-up"); },
  });
  scheduler.submit({
    canonicalRoot: "/repo-a",
    reason: "reconcile",
    run: async () => { runs.push("lower-priority"); },
  });
...
test("scheduler cancels queued jobs and lets a running job finish during close", async () => {
  ...
  const running = scheduler.submit({
    canonicalRoot: "/repo-a",
    reason: "manual",
    run: () => new Promise((resolve) => { releaseRunning = resolve; }),
  });
