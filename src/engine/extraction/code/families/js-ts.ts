...
export function shouldIndexJavascriptTypescriptEntity(node: TSNode): boolean {
  ...
  if (node.type === "pair") {
    return hasFunctionValue(node) && exportedObjectVariableName(node) !== undefined;
  }
  ...
  if (node.type === "variable_declarator" && exportedObjectFunctionEntities(node).length > 0) {
    return true;
  }
...
export function resolveJavascriptTypescriptEntities(node: TSNode): readonly TSNode[] {
...
export function extractJavascriptTypescriptName(node: TSNode): string | undefined {
  if (node.type === "pair") {
...
export function extractJavascriptTypescriptSignature(node: TSNode): string | undefined {
  if (node.type === "pair") {
    const key = extractJavascriptTypescriptName(node);
    const value = node.childForFieldName("value");
    const valueSignature = value ? extractGenericSignature(value) : undefined;

    return key && valueSignature ? `${key}: ${valueSignature}` : extractGenericSignature(node);
  }
...
export function classifyJavascriptTypescriptNode(node: TSNode): CodeSymbolType | undefined {
  if (
    node.type === "pair"
...
function hasFunctionValue(node: TSNode): boolean {
  const value = node.childForFieldName("value")
    ?? node.namedChildren.find((child) => JS_TS_FUNCTION_VALUE_TYPES.has(child.type));
...
function exportedObjectFunctionEntities(node: TSNode): TSNode[] {
  ...
  return value.namedChildren.filter((child) =>
...
function exportedObjectVariableName(node: TSNode): string | undefined {
  const object = closestAncestor(node, "object") ?? closestAncestor(node, "object_expression");
  const variable = object?.parent;

  if (!variable || variable.type !== "variable_declarator" || !isExportedVariableDeclarator(variable)) {
    return undefined;
  }
...
function isObjectMember(node: TSNode): boolean {
  return node.parent?.type === "object" || node.parent?.type === "object_expression";
}
