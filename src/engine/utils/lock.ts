...
function acquireExclusiveDirectoryLock(lockPath: string, options: FileLockOptions): FileLock {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      writeFileSync(lockInfoPath(lockPath), `${JSON.stringify(info, null, 2)}\n`, "utf8");
      ...
      if (cleanupStaleLock(lockPath, options.staleMs ?? DEFAULT_STALE_LOCK_MS)) {
        continue;
      }
...
function assertExclusiveDirectoryUnlocked(lockPath: string, operation: string): void {
...
function acquireReadLock(lockPath: string, options: FileLockOptions): FileLock {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const info = currentLockInfo(options.operation);
    const readerPath = join(readersLockPath(lockPath), `${info.pid}-${info.token}`);
    try {
      mkdirSync(readerPath, { recursive: true });
      writeFileSync(lockInfoPath(readerPath), `${JSON.stringify(info, null, 2)}\n`, "utf8");
...
function acquireWriteLock(lockPath: string, options: FileLockOptions): FileLock {
  const staleMs = options.staleMs ?? DEFAULT_STALE_LOCK_MS;
  const writePath = writeLockPath(lockPath);
...
function isStaleLock(lockPath: string, info: FileLockInfo | null, staleMs: number): boolean {
  const now = Date.now();
...
function readLockInfo(lockPath: string): FileLockInfo | null {
  try {
    const parsed = JSON.parse(readFileSync(lockInfoPath(lockPath), "utf8")) as Partial<FileLockInfo>;
    if (
...
function lockBusyError(lockPath: string, requestedOperation: string): EngineError {
...
function readLockBusyError(lockPath: string, requestedOperation: string): EngineError {
