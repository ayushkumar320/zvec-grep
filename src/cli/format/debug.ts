...
export function printDebug(
  ...
  console.error(`source=${result.source} coverage=${result.coverage} items=${result.items.length}`);

  if (result.collection) {
    console.error(`collection=${result.collection.name} anonymous=${result.collection.anonymous} path=${result.collection.path}`);
  }
  ...
  if (result.diagnostics.index) {
    const routes = result.diagnostics.index.routes
      ...
      .join(", ");
    console.error(`index_hits=${result.diagnostics.index.hitsReturned} routes=${routes}`);
  }

  if (result.diagnostics.fallback) {
    const fallback = result.diagnostics.fallback;
    console.error(`fallback=${fallback.backend} limit=${fallback.limit ?? "all"} truncated=${fallback.truncated}`);
    console.error(`fallback_command=${shellArg(fallback.command)} ${fallback.args.map(shellArg).join(" ")}`);
  ...
  if (result.diagnostics.timings && result.diagnostics.timings.length > 0) {
    console.error(`timings=${result.diagnostics.timings.map(formatTiming).join(" ")}`);
  }
...
function formatTiming(entry: NonNullable<ZvecGrepContextResult["diagnostics"]["timings"]>[number]): string {
