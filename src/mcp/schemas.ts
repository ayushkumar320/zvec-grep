...
const boundedString = (description: string) => z.string()
  .max(MCP_MAX_QUERY_CHARS)
  .describe(description);

const boundedStringList = (description: string) => z.union([
...
const legacyStringListInputSchema = z.union([z.string(), z.array(z.string())]).optional();
const legacyTimeInputSchema = z.union([z.number().int().nonnegative(), z.string()]).optional();

export const stringListInputSchema = boundedStringList("A string or a list of strings.");
export const pathFilterInputSchema = z.union([
  pathFilter,
  z.array(pathFilter).max(MCP_MAX_PATH_FILTERS),
]).optional();
...
export const absoluteRootSchema = z.string()
  .trim()
...
const searchFields = {
  ...
  queries: boundedStringList("Multiple query groups. Use this for related concepts."),
  fts: boundedStringList("Exact lexical anchors, such as symbols, flags, or error messages."),
  vector: boundedStringList("Explicit semantic/vector-only queries."),
  limit: z.number().int().positive().max(MCP_MAX_SEARCH_LIMIT).optional()
  ...
  symbolTypes: z.array(codeSymbolTypeSchema).max(6).default([])
    .describe("Restrict indexed results to symbol types."),
  modifiedAfter: timeInputSchema.describe("Only query files modified after this time."),
  modifiedBefore: timeInputSchema.describe("Only query files modified before this time."),
  trace: z.boolean().optional().describe("Include per-hit search trace in structured output."),
  maxContentChars: z.number().int().positive().max(MCP_MAX_CONTENT_CHARS).default(1_200)
...
const searchResultSchema = z.object({
  ...
  coverage: z.enum(["ranked_sample", "lexical_exhaustive", "lexical_truncated"]),
  collection: z.object({
    id: z.string(),
    name: z.string(),
    path: z.string(),
  ...
  diagnostics: z.object({
    emptyReason: z.enum(["no_matches", "no_searchable_files", "index_unavailable", "search_failed"]).optional(),
...
export const legacySearchInputSchema = z.object({
  ...
  queries: legacyStringListInputSchema.describe("Multiple query groups. Use this for related concepts."),
  fts: legacyStringListInputSchema.describe("Exact lexical anchors, such as symbols, flags, or error messages."),
  vector: legacyStringListInputSchema.describe("Explicit semantic/vector-only queries."),
  ...
  include: legacyStringListInputSchema.describe("Glob filters for paths to include. Accepts an array or CLI-style comma-separated string."),
  exclude: legacyStringListInputSchema.describe("Glob filters for paths to exclude. Accepts an array or CLI-style comma-separated string."),
  preferSymbol: z.boolean().optional().describe("Prefer exact indexed symbols when query names a symbol."),
  symbolTypes: z.array(codeSymbolTypeSchema).default([]).describe("Restrict indexed results to symbol types."),
  modifiedAfter: legacyTimeInputSchema.describe("Only query files modified after this time. Accepts epoch milliseconds or a parseable date string."),
  modifiedBefore: legacyTimeInputSchema.describe("Only query files modified before this time. Accepts epoch milliseconds or a parseable date string."),
  autoUpdate: z.boolean().optional()
    .describe("Refresh an existing stale anonymous index before query. Defaults to CLI behavior: true."),
  embeddingConcurrency: z.number().int().positive().max(64).optional()
    .describe("Embedding task concurrency for automatic index refresh."),
  trace: z.boolean().optional().describe("Include per-hit search trace in structured output."),
  maxContentChars: z.number().int().positive().max(6_000).default(1_200)
    .describe("Maximum source characters to include per hit."),
});

export const legacyRgInputSchema = z.object({
  pattern: z.string().optional().describe("Regex or literal pattern to search for."),
  patterns: legacyStringListInputSchema.describe("Multiple regex or literal patterns to search for."),
  root: z.string().optional().describe("Repository root. Defaults to the MCP server working directory."),
  paths: legacyStringListInputSchema.describe("Optional paths to search within the root."),
  ...
  beforeContext: z.number().int().nonnegative().max(20).optional()
    .describe("Context lines before each match."),
  afterContext: z.number().int().nonnegative().max(20).optional()
    .describe("Context lines after each match."),
  hidden: z.boolean().optional().describe("Search hidden files and directories."),
  glob: legacyStringListInputSchema.describe("ripgrep glob filters, for example '*.ts' or '!dist/**'. Accepts an array or CLI-style comma-separated string."),
...
export type ZvecGrepIndexStatusInput = z.infer<typeof zvecGrepIndexStatusInputSchema>;
